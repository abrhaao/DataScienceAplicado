...
public class DataSourceEspn implements DataSource, Serializable {
	 ...
	 * @param dsTemporada
	 */
	public DataSourceEspn(String dsCampeonato, String dsTemporada) {
		super();
	...
	public TreeMap<String, URL> getClubes(URL urlEquipes) throws IOException {
		...
		PrintWriter writer = new PrintWriter("/tmp/hivedados/papli.db/paginas/" + "campeonato-espn-" + this.dsCampeonato.replace("/","") + ".html", "UTF-8");
		...
		if ( urlEquipes.toString().contains("espn") ) {
			...
			for (Element e: eClubes) {
				if ( e.select("a").size()  > 0 ) { 
					...
					String[] clube = { e.select("abbr").attr("title"), "http://localhost:8000" + linkElenco + "/dados.html" };
	...
	public TreeMap<String, URL> getAtletas(String nomeClube, URL urlAtletas, PrintWriter writer) throws IOException {
		...
		Document docHtml = Jsoup.parse(htmlContent.toString());
		PrintWriter wPagina = new PrintWriter("/tmp/hivedados/papli.db/paginas/" + "campeonato-espn-" + this.dsCampeonato.replace("/","") + "-" + nomeClube + ".html", "UTF-8");
			...
			//if ( urlAtletas.toString().contains("espn") ) {
				...
				for (Element e: eJogadores) {
					...
					try {
						...
						Elements caracteristicas = e.select("span");	//mudou, cacete!!!!
						...
						for ( int ix = 0; ix<caracteristicas.size(); ix++) {
							...
							if ( e.select("span").get(1).text().equals("G")) {
								switch (ix) {
								...
								switch (ix) {
								case 2: dataCaracteristicas.append("<Idade>" + s + "</Idade>"); break; 
								case 3: dataCaracteristicas.append("<Jogos>" + s + "</Jogos>"); break; 
								case 5:	dataCaracteristicas.append("<Gols>" + s + "</Gols>"); break;																								
								case 6: dataCaracteristicas.append("<Assistencias>" + s + "</Assistencias>"); break;
								case 7: dataCaracteristicas.append("<Finalizacoes>" + s + "</Finalizacoes>"); break;
								case 8: dataCaracteristicas.append("<FinalizacoesCertas>" + s + "</FinalizacoesCertas>"); break;
								case 9: dataCaracteristicas.append("<Faltas>" + s + "</Faltas>"); break;
								case 10: dataCaracteristicas.append("<FaltasSofridas>" + s + "</FaltasSofridas>"); break;
								case 11: dataCaracteristicas.append("<CardsAmarelo>" + s + "</CardsAmarelo>"); break;
								case 12: dataCaracteristicas.append("<CardsVermelho>" + s + "</CardsVermelho>"); break;
								}
							}
						}						
